...
// The shared committee password for the test DB only — this database is wiped
// every run and never leaves the machine, so a fixed value keeps adminLogin()
// free of env setup. Production's password is generated by
// `scripts/admin.js --rotate-password` and lives nowhere but a bcrypt hash.
const E2E_ADMIN_PASSWORD = 'e2e-shared-admin-password';
...
async function seedTestDb() {
  ...
  await db.systemConfig.create({
    data: {
      ...
      // Shared admin password — what /api/admin/login checks for every admin.
      adminPasswordHash: await bcrypt.hash(E2E_ADMIN_PASSWORD, 12),
    },
  });
  ...
  // Admin — no personal passwordHash on purpose: this account logs in the way a
  // committee member does, with its studentId + the shared password above.
  const admin = {
...
module.exports = { seedTestDb, ADMIN_STUDENT_ID, ADMIN_EMAIL, E2E_ADMIN_PASSWORD };
