...
- **F5 · คำต้องห้ามใหม่ (กติกาถาวรทั้งระบบ — เข้า taste constitution):**
  ❌ "คูหา" ❌ "หีบ" ในทุก copy หน้า public — ทางการโบราณเกิน → ใช้
  **"โหวต" / "ลงคะแนน"** (ปิดโหวตแล้ว, เปิดโหวตใน, ไปลงคะแนน)
  **owner เคาะ 2026-07-16 (AskUserQuestion):** (1) ขอบเขต = **ทุก template หน้า
  public ทั้ง 6 ตระกูล** (admin config labels ไม่รวม) (2) **ไม่มีข้อยกเว้น** —
  ฉากหย่อนหีบก็ต้องเปลี่ยนคำ (ตัว animation คงไว้ แค่เปลี่ยน copy: "หย่อนบัตร →" /
  "กล่องรับบัตร") (3) concept "คิว" ก็ไม่เอา (เราไม่มีคิวจริง) — queue slip เปลี่ยน
  เป็นภาษาโหวต/ลงคะแนน
