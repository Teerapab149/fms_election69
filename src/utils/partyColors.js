 ...
 *
 * Default = a "rival" palette of contrasting pairs (party 1 blue vs party 2 red,
 * head-to-head; then more contrasting hues as parties are added). An admin can
 * override per party (Candidate.color); that custom hex is then ADAPTED toward the
 * soft gumroad tone so a loud pick still sits in the gentle palette.
 *
 * Colours are tuned to the soft "ละมุน" system tone (pastel-strong, readable with
 * ink #26271c text + borders).
 */
 ...
 * very saturated/dark admin picks soften into the pastel-strong range. Keeps the
 * hue, so the party's chosen identity is preserved, just gentler.
 */
...
export function getPartyColor(party, index = 0) {
  ...
  if (custom) return adaptToTone(custom);
