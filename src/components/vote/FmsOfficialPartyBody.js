...
// FmsOfficialPartyBody — the party dossier itself: identity notice, crest
// reading, vision, policies, team, gallery. Everything a party's case is made
// of, and nothing about the page it sits on.
//
// It exists as its own component because TWO screens present a party in this
// template — /party, and the single-party ballot, where a voter is asked to
// endorse or reject and has to see the case before deciding. The alternative
// was copying the markup and its ~200 lines of CSS into the second screen, and
// this family has already shipped four devices unstyled by declaring their
// rules in a file the rendering page never loads. Styles that travel with the
// markup cannot come apart from it.
//
...
export const resolveSrc = (p) => (!p ? null : (String(p).startsWith("http") ? p : getPath(p)));

export default function FmsOfficialPartyBody({
  party = {}, galleryImages = [], editorMode = false,
}) {
  ...
  return (
    <>
      ...
      {story.length > 0 && (
        <section className="fo-party__sec">
          ...
          {/* The longest prose in the template, and the least load-bearing: a
              ...
              The panel runs the page's full measure, with the crest taking the
              width the text cannot use: 1092 at 15px is about 124 Thai
              characters a line, and this section is the one thing that should
              obviously show the crest it is explaining. */}
      ...
      <style jsx global>{`
        ...
        /* .fo-party__sec (the section rhythm) lives in FmsOfficialShell — three
           components set it now, so it belongs to the shared frame */

        /* the crest panel's text column holds the measure now, so the paragraph
           itself no longer caps: 638 at 1440, about 72 Thai characters a line */
        ...
        /* ── the crest reading ──
           ...
           control's radius, same as the notice. The crest takes a fixed 400 and
           the reading gets the rest, so the text measure falls out of the page
           width rather than being set by hand. */
        .fo-party__story {
        ...
        @media (max-width: 760px) {
          ...
          /* The crest drops rather than stacking above the reading. It is already
             ...
             The viewport stays well under the screen height on purpose: a scroll
             box that fills the screen steals the page's own scroll on a phone. */
