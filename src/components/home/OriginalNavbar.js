  ...
  return (
    <nav className="sticky top-0 z-50 w-full bg-white/80 backdrop-blur-md border-b border-gray-100 shadow-sm transition-all duration-300">
      <div className="container mx-auto px-4 md:px-6 py-3 flex justify-between items-center">
            ...
            <div className="relative ml-2 pl-4 border-l border-gray-200" ref={dropdownRef}>
              ...
              >
                <div className="hidden xl:block text-right mr-1">
                  <p className="text-sm font-bold text-gray-700 group-hover:text-[var(--o-brand)] leading-none transition-colors">{user.name}</p>
                  <p className="text-[10px] text-gray-400 font-medium">{user.studentId || user.email}</p>
                </div>
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[var(--o-brand)] to-[var(--o-mid)] flex items-center justify-center text-white shadow-md group-hover:shadow-lg transition-all transform group-hover:scale-105">
                  <User size={20} />
                </div>
        ...
        {/* ================= Hamburger Button ================= */}
        <button
          className="lg:hidden p-2 text-[var(--o-brand)] rounded-xl hover:bg-[var(--o-soft)] transition-colors focus:outline-none"
      ...
      <div className={`lg:hidden absolute top-full left-0 w-full bg-white/95 backdrop-blur-xl border-b border-gray-200 shadow-xl transition-all duration-300 ease-in-out origin-top overflow-hidden ${isMenuOpen ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="flex flex-col p-4 space-y-2 pb-6">
          ...
          {isLoggedIn && (
            <div className="bg-gradient-to-r from-[var(--o-soft)] to-white p-4 rounded-2xl mb-4 border border-[var(--o-soft2)] flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-[var(--o-brand)] flex items-center justify-center text-white shadow-sm border border-grey-200">
                <User size={24} />
              </div>
              <div>
  ...
  return (
    <Link
      ...
      className={`
        ...
        ${isActive
          ? 'bg-[var(--o-soft)] text-[var(--o-brand)]'
          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
        }
      `}
    >
      <span className={`absolute left-0 top-0 h-full w-1.5 bg-[var(--o-brand)] rounded-r-full transition-all duration-300 ease-out origin-left
