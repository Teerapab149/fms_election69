...
### ⚖️ การตัดสินผลเลือกตั้ง — `src/utils/electionVerdict.js` เท่านั้น
`resolveVerdict(candidates, { revealed })` คือที่เดียวที่ตัดสินว่าใครชนะ/รับรองไหม
**ห้ามคำนวณผู้ชนะเองในไฟล์ธีม** (เคยทำแบบนั้นแล้วผลเลือกตั้งชุดเดียวกันให้คำตอบต่างกัน
6 แบบตามธีมที่แอดมินเลือก — Gumroad ตัด "ไม่รับรอง" ออกจากสมการ, Studio/Verdure ใช้ `>=`
เลยรับรองพรรคที่ 0 เสียง, Blossom/Receipt เสมอกันแล้วเอาตัวแรกใน array, FmsOfficial
ให้ถ้วยงดออกเสียงได้) ธีมมีหน้าที่ "แสดงผลที่ตัดสินมาแล้ว" ไม่ใช่ "ตัดสิน"

กติกาที่ไฟล์นั้นยึด: งดออกเสียงไม่เข้า pool ตัดสิน · ไม่รับรองเข้าเฉพาะบัตรพรรคเดียว ·
ไม่มีคะแนนเลย = `no-votes` ไม่ใช่รับรอง · **เสมอกัน = ไม่ประกาศใคร (`tie`)**
⚠️ ข้อสุดท้ายรอคณะกรรมการยืนยัน ถ้าระเบียบสโมฯ มีวิธีตัดสินเสมอ ให้แก้ที่ไฟล์นั้นไฟล์เดียว

### 🗳️ แก้โครงสร้างบัตรหลังเริ่มโหวตแล้ว — ทำไม่ได้
`ballotBoxGuard()` ใน `api/admin/candidates/route.js` ห้ามเพิ่ม/ลบพรรค และห้ามเปลี่ยน
`number` เมื่อมีแถวใน `Ballot` แล้ว หรือเมื่อรับรองผลแล้ว (`globalConfig.ballotsAnonymized`)
เพราะบัตรเป็น ciphertext ของ `{ c: candidateId }` ที่ย้อนกลับไปนับใหม่ไม่ได้
แก้เนื้อหา (สโลแกน/รูป/นโยบาย) ยังทำได้ปกติ

### 🧹 การรัน patch script กับไฟล์ในรีโปนี้
ไฟล์ส่วนใหญ่เป็น **CRLF** — `String.replace()` แบบหลายบรรทัดจะไม่ match เงียบ ๆ
สคริปต์จะรายงานว่าสำเร็จทั้งที่ไม่มีอะไรเปลี่ยน ท่าที่ใช้ได้: อ่าน → `.replace(/
/g,'
')`
→ แก้ → เขียนกลับด้วย `.replace(/
/g,'
')` **และต้อง `if (s === before) process.exit(1)` เสมอ**
