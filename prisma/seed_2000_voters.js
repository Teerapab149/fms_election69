...
// const bcrypt = require('bcryptjs'); // Removed for Docker compatibility
const prisma = new PrismaClient();

async function main() {
    console.log('🌱 Start seeding (2,000 Unvoted Users Template)...');
    ...
    // 3. ข้อมูลสมาชิกพรรค SAMO (21 คน จากไฟล์ที่อัปโหลด)
    const samoMembers = [
    ...
    const partiesData = [
        {
            ...
            // 2. Group Image (Corrected path based on actual file found)
            groupImages: ['/images/candidates/groupimage/party1/GROUP_The_Unity_Concord_Of_FMS_2_1769963102478_0.jpg'],
            // 3. Mobile Hero Vertical (Use 1.png if it exists, otherwise keep as is or null)
            mobileHeroImages: ['/images/candidates/mobileheroimage/party1/1.png'],
            // 4. Official Hero Section (Use actual file found)
            officialImageUrl: '/images/candidates/officialImageUrl/OFFICIAL_The_Unity_Concord_Of_FMS_2_1769963184354.jpg',
...
ดวงดาวทั้ง 9 ดวง เปรียบเสมือน 9 สาขาวิชาของคณะวิทยาการจัดการ ทำหน้าที่เป็นแสงนำทางให้แก่สิงห์สำเภา ขณะที่เรือสีม่วงสะท้อนถึงจิตวิญญาณของคณะวิทยาการจัดการที่มีความมั่นคง แข็งแกร่ง และพร้อมเผชิญทุกสถานการณ์`,
            ...
            policies: [
                'ยกระดับโครงการเดิมและพัฒนาโครงการใหม่เพื่อปรับเปลี่ยนรูปแบบและแนวคิดให้มีความเหมาะสมกับยุคสมัยมากขึ้น',
                'ส่งเสริมกิจกรรมด้านความคิดสร้างสรรค์ และเปิดโอกาสให้นักศึกษาได้แสดงออกอย่างเหมาะสม ควบคู่กับการเสริมสร้างสุขภาพจิตที่ดีของนักศึกษาคณะวิทยาการจัดการ',
                'สร้างพื้นที่ในการแสดงออกความคิดเห็นและข้อเสนอแนะเพื่อนำมาพัฒนาและปรับปรุงการจัดกิจกรรมต่าง ๆ ให้เกิดประโยชน์สูงสุดแก่นักศึกษาคณะวิทยาการจัดการ',
                'สร้างเครือข่ายชุมชนและองค์กรที่มุ่งเน้นความหลากหลายและการอยู่ร่วมกันอย่างสันติ ผ่านการจัดกิจกรรมเสริมสร้างความเข้าใจในวัฒนธรรมที่หลากหลาย'
            ]
        },
        {
            name: 'งดออกเสียง',
    ...
    // Map for irregularities in Modal filenames
    const modalImageMap = {
        8: '8_6810517021.jpg',
        9: '9_6810515043.jpg'
    };
    ...
    for (const p of partiesData) {
        ...
        if (p.members.length > 0) {
            for (const m of p.members) {
                ...
                // Modal - Default to .jpg unless present in map
                const modalFilename = modalImageMap[m.no] || `${m.no}.jpg`;
                const modalImagePath = `/images/members/party_${p.number}/Modal/${modalFilename}`;
    ...
    // 6. เสก User 2,000 คน (Voters - All Unvoted)
    const votersCount = 2000;
    console.log(`Populating ${votersCount} UNVOTED voters...`);
    ...
    const usersDataBatch = [];

    for (let i = 0; i < votersCount; i++) {
        const selectedMajor = majorData[Math.floor(Math.random() * majorData.length)];
        const randomYear = years[Math.floor(Math.random() * years.length)];
        const randomGender = genders[Math.floor(Math.random() * genders.length)];
        const runningNumber = String(i).padStart(4, '0');
        const realStudentId = `${yearPrefixMap[randomYear] || '64'}1051${runningNumber}`;
    ...
    console.log(`✅ ${votersCount} UNVOTED Voters created.`);

    // 7. สร้าง Admin Users
    console.log('✨ Creating Admins...');

    // สร้าง Admin Password Hash (Hardcoded for Docker compatibility)
    // const adminRawPassword1 = "6610510149@email.psu.ac.th+ADMIN_FMS2026_2026_secret_9QpZxL";
    const adminPasswordHash1 = "$2b$12$cW6J1oa7l6ZVZNnVE/Y1VuDhQSsWvN5lc.a0DVTmUYERHQMJ1lN0e"; // Pre-calculated

    // const adminRawPassword2 = "6610510129@email.psu.ac.th+ADMIN_FMS2026_2026_secret_9QpZxL";
    const adminPasswordHash2 = "$2b$12$5aeiQRDCuT29wkWY7W24s.6/d7dRf5ykmgbjTylKteZTXltiFILeC"; // Pre-calculated
